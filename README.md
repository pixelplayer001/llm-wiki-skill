...
## Sample digest (DeepSeek)

This repository includes a **categorized markdown digest** of a long DeepSeek technical chat (robotics, automotive, drones, RC), with a topic inventory and **8** articles under [`my-wiki/raw/articles/`](my-wiki/raw/articles/) (`topic-inventory.md` plus `mobility/`, `drones/`, `rc/`, `language/`). Source: user-supplied PDF export and [this share link](https://chat.deepseek.com/share/krcvr538zt632sy0c4) (reference only).

## Install

### Cursor

This repository includes a **Cursor Agent skill** at [`.cursor/skills/llm-wiki/SKILL.md`](.cursor/skills/llm-wiki/SKILL.md). Open the repo as your workspace, enable project skills (or invoke `@llm-wiki` / your Cursor UI equivalent), and follow that file — it points at `llm-wiki/scripts/` and `llm-wiki/references/` in this tree. To reuse the skill in another project, copy the folder `.cursor/skills/llm-wiki/` into that project’s `.cursor/skills/` directory.

### Claude / Codex (global skills dir)
...
Then reference it in your agent config, or paste `llm-wiki/SKILL.md` into your agent context. For Cursor, prefer the project skill above instead of duplicating `llm-wiki/SKILL.md` under `~/.claude/skills/`.
